...
combined %>% 
    filter(., Line_ID %in% c("0","2","3","4","5","6","7","8","9","11","12","13","14","15","16","17")) %>% 
  filter(., `Depth (m)` < 100) %>% 
...
combined %>% 
  ...
  filter(., `Depth (m)` < 100) %>% 
    group_by(Line_ID) %>% 
  dplyr::summarise(n_distinct(Station_ID))

combined %>% 
  filter(., Line_ID %in% c("2","5","12","15","17")) %>% 
  filter(., `Depth (m)` < 100) %>% 
...
ggplot(data = world) +
    ...
    coord_sf(xlim = c(-127, -117), ylim = c(46, 50), expand = FALSE) +theme_bw() +xlab("Longitude") +ylab("Latitude") +scale_color_manual(values=c(pal,pal2,pal3,pal4)) +
